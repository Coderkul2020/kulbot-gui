...
// The tests below require Suny Link to be unavailable, so we can trigger
...
describe('Hardware extension connection modal', () => {
    ...
    test('Message saying Suny Link is unavailable (BLE)', async () => {
        await driver.quit();
        driver = getDriver();
        ...
        findByText('Suny Link'); // Suny Link is mentioned in the error modal

        const logs = await getLogs();
        await expect(logs).toEqual([]);
    });

    test('Message saying Suny Link is unavailable (BT)', async () => {
        await loadUri(uri);
